...
// 1. env EXPO_PUBLIC_API_URL (preferred for TestFlight/production)
// 2. app.json -> expo.extra.apiUrl
// 3. Dev: try to derive LAN host from hostUri (when using LAN / tunnel) and assume port 3000
// 4. Fallback to localhost (only works on emulator / simulator)
...
const configApiUrl = (Constants as any)?.expoConfig?.extra?.apiUrl as
  | string
  | undefined;
const BASE_URL = (
  envApiUrl ||
...
export const wordsApi = {
  ...
  due: (languageCode?: string) =>
    api.get<any>(
      `/api/learning/due-words${languageCode ? `?languageCode=${languageCode}` : ""}`,
    ),
  create: (w: {
    word: string;
    definition?: string;
...
export const quizApi = {
  ...
  current: (sessionId: string) =>
    api.get<any>(`/api/learning/quiz?sessionId=${sessionId}`),
  answer: (sessionId: string, answer: string, responseTimeMs?: number) =>
    api.post<any>("/api/learning/quiz/answer", {
      sessionId,
      answer,
      responseTimeMs,
    }),
...
export const authApi = {
  login: (identifier: string, password: string) =>
    api.post<any>("/api/auth/login", { identifier, password }),
...
export const postsApi = {
  listUser: (identifier: string, limit = 20) =>
    api.get<any>(
      `/api/posts?identifier=${encodeURIComponent(identifier)}&limit=${limit}`,
    ),
  following: (limit = 20) =>
    api.get<any>(`/api/posts/following?limit=${limit}`),
