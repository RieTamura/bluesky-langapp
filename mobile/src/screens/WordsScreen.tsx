...
export const WordsScreen: React.FC = () => {
  ...
  const [examples, setExamples] = React.useState<{
    word: string;
    examples: { id: string; text: string; translation?: string }[];
  } | null>(null);
  ...
  const syncedWords = React.useMemo(
    () => words.filter((w) => w.id && !w.id.startsWith("temp_")),
    [words],
  );
  const unregisteredLocalCount = React.useMemo(
    () => words.filter((w) => w.id && w.id.startsWith("temp_")).length,
    [words],
  ); // 並び順要求: 1.復習 2.LEARNING 3.KNOWN 4.UNKNOWN 5.ステータス(グループ表示) 6.A-Z
  const [sortKey, setSortKey] = React.useState<
    "review" | "learning" | "known" | "unknown" | "status" | "alpha"
  >("review");

  type ListRenderable =
    | { type: "word"; data: (typeof words)[number] }
  ...
  const listData: ListRenderable[] = React.useMemo(() => {
    ...
    const base =
      syncedWords && syncedWords.length > 0 ? [...syncedWords] : [...words];
      ...
      switch (sortKey) {
        ...
        case "review":
          return base.sort((a, b) => {
            ...
            const ca = a.correctCount || 0;
            const cb = b.correctCount || 0;
    ...
    if (
      ...
      return sorted
        .filter((w) => w.status === sortKey)
        .map((w) => ({ type: "word", data: w }) as ListRenderable);
    }
    if (sortKey !== "status") {
  ...
  const fetchAndShowExamples = React.useCallback(async (wordStr: string) => {
    try {
      ...
      const ex = (await dict.fetchExampleSentences(wordStr)) as Array<{
        id: string;
        text: string;
        translation?: string;
      }>;
      setExamples({
        word: wordStr,
        examples: ex.map(
          (e: { id: string; text: string; translation?: string }) => ({
            id: e.id,
            text: e.text,
  ...
  const speak = async (text: string) => {
    ...
    try {
      ...
      M.speak(text, { language: "en" });
    } catch (e) {
      // ignore TTS errors to avoid crashing
    }
  };
  ...
  const toggleFilters = () => {
    if (showFilterModal) closeFilters();
    else openFilters();
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]}>
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ marginLeft: 8 }}>
            <Text
              style={{
                color: pq > 0 ? c.accent : c.secondaryText,
                fontSize: 14,
                fontWeight: "700",
              }}
            >
            ...
            {!!lastSyncAt && (
              <Text
                style={{ color: c.secondaryText, fontSize: 11 }}
              >{`最終同期: ${new Date(lastSyncAt).toLocaleString()}`}</Text>
            )}
            {unregisteredLocalCount > 0 && (
              <Text
                style={{ color: c.accent, fontSize: 12, marginTop: 2 }}
              >{`ローカル未登録 ${unregisteredLocalCount} 件`}</Text>
            )}
          </View>
        </View>
      ...
      {examples && (
        <View style={{ paddingHorizontal: 16, paddingVertical: 8 }}>
          ...
          >{`例文（${examples.word}）`}</Text>
          {examples.examples.map((ex) => (
            <View key={ex.id} style={{ marginBottom: 8 }}>
              <Text style={{ color: c.text }}>{ex.text}</Text>
              {ex.translation ? (
                <Text style={{ color: c.secondaryText, fontSize: 12 }}>
                  {ex.translation}
                </Text>
              ) : null}
              <View style={{ flexDirection: "row", marginTop: 6 }}>
                <TouchableOpacity
                  onPress={() => speak(ex.text)}
                  style={{
                    marginRight: 8,
                    padding: 8,
        ...
        renderItem={({ item }) => {
          ...
          return (
            <WordCard
              ...
              onPress={() => fetchAndShowExamples(item.data.word)}
            />
          );
        }}
      />
      ...
      {paid === false && (
        ...
        >
          ...
          >
            <Text style={{ color: c.secondaryText, textAlign: "center" }}>
              広告枠（ここにAdMobバナーを導入）
            </Text>
          </View>
        </View>
...
const SortTabs: React.FC<{
  current: "review" | "learning" | "known" | "unknown" | "status" | "alpha";
  onChange: (
    k: "review" | "learning" | "known" | "unknown" | "status" | "alpha",
  ) => void;
}> = ({ current, onChange }) => {
  const c = useThemeColors();
  const options: {
    key: "review" | "learning" | "known" | "unknown" | "status" | "alpha";
    label: string;
  }[] = [
    { key: "review", label: "復習" },
