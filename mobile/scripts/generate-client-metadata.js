...
const envUrl =
  process.env.CLIENT_METADATA_URL ||
  process.env.CLIENT_ID ||
  process.env.CLIENT_ID_URL;
...
try {
  ...
  if (Array.isArray(extra.redirectUris) && extra.redirectUris.length)
    defaultRedirects = extra.redirectUris;
} catch (e) {
  // ignore and use hardcoded defaults
}

const normalizeToMetadataUrl = (url) => {
  if (!url) return url;
  try {
    const u = new URL(url);
    if (u.pathname.endsWith("/.well-known/atproto_client_metadata.json"))
      return u.toString();
    const base = url.replace(/\/+$/, "");
    return `${base}/.well-known/atproto_client_metadata.json`;
  } catch (e) {
    return url;
  }
};

const clientId = normalizeToMetadataUrl(envUrl) || defaultUrl;
...
const template = {
  ...
  redirect_uris: [...defaultRedirects.slice(0, 2), defaultAuthProxy],
