...
async function initHandler(c) {
  const { BSKY_ISSUER, CLIENT_ID, REDIRECT_URI, DEV_REDIRECT_URI, SCOPE } = c.env;
  const useDevRedirect = (c.req.query("redirect") || "").toLowerCase() === "dev" && !!DEV_REDIRECT_URI;
...
app.post("/api/atprotocol/token", async (c) => {
    ...
    return c.json(
      {
        ...
        message: "code must be the short authorization code, not the authorize URL"
      },
      400
    );
  }
  if (code.length > 2048) {
    return c.json(
      {
        error: "VALIDATION_ERROR",
        message: "code is unexpectedly long"
      },
      400
    );
  }
  ...
  const { code_verifier, redirect_uri, client_id } = JSON.parse(pkceRaw);
  const { publicKey, privateKey } = await generateKeyPair2("ES256");
  ...
  }).setProtectedHeader({ alg: "ES256", typ: "dpop+jwt", jwk: pubJwk }).sign(privateKey);
  const form = new URLSearchParams();
  form.set("grant_type", "authorization_code");
  form.set("code", code);
  form.set("redirect_uri", redirect_uri || c.env.REDIRECT_URI);
  form.set("client_id", c.env.CLIENT_ID);
    ...
    return c.json(
      {
        ...
        message: json?.error_description || "Token exchange failed",
        status: resp.status
      },
      resp.status
    );
  ...
  const token_type = json?.token_type || "DPoP";
...
app.get("/dev/callback", (c) => {
  ...
  const escaped = /* @__PURE__ */ __name((s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"), "escaped");
...
<body>
  <h1>Dev Callback</h1>
  ${error ? `<div class="box err"><strong>Error:</strong> ${escaped(
    error
...
// .wrangler/tmp/bundle-ElfZha/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ElfZha/middleware-loader.entry.ts
