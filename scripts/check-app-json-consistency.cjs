...
// Files or directories to ignore (docs and data copies are valid canonical or generated files)
const ignorePaths = [
  path.join(repoRoot, 'docs'),
  path.join(repoRoot, 'mobile', 'client-metadata.json'),
  path.join(repoRoot, 'client-metadata.json'),
  path.join(repoRoot, 'scripts', 'check-app-json-consistency.js'),
  path.join(repoRoot, 'scripts', 'check-app-json-consistency.cjs')
];
...
function searchRepo() {
  ...
  const hits = [];
  for (const f of files) {
    // skip node_modules and ignored paths
    if (f.includes('node_modules')) continue;
    let skip = false;
    for (const ig of ignorePaths) {
      if (f.startsWith(ig)) { skip = true; break; }
    }
    if (skip) continue;
    let content;
